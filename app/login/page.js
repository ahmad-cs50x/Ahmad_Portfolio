...
const ERROR_MESSAGES = {
  ...
  EmailSignin:
    "Could not send the sign-in link. Check your SMTP settings — Gmail needs a 16-character App Password.",
  ...
  async function handleEmailSubmit(e) {
    ...
    try {
      ...
      toast.error("Failed to send sign-in link", {
        detail: "Please check your SMTP configuration and try again.",
      });
      setBusy(false);
    }
  }
  ...
  async function handleCredentialsSubmit(e, autoEmailParam, autoPasswordParam) {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // result.url is the canonical post-auth destination NextAuth computed
        // for this credentials callback (resolves callbackUrl). Prefer it, but
        // fall back to callbackUrl if the client ever returns undefined.
        ...
        <motion.div {...enter} className="relative z-10 mx-auto w-full max-w-[400px]">
          ...
          {!providers ? (
            ...
            <div className="mt-8 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-5 text-sm leading-relaxed text-amber-200">
              ...
              (Google OAuth and/or SMTP) and restart the server.
            </div>
          ) : (
            <>
              {hasGoogle && (
